...
SLOT="0"
KEYWORDS="*"
IUSE="doc emacs examples matplotlib mongodb nbconvert notebook octave qt4 readline +smp sqlite test wxwidgets"
...
RDEPEND="$(python_abi_depend dev-python/decorator)
	...
	notebook? (
		dev-libs/mathjax
		$(python_abi_depend -e "*-pypy-*" dev-python/jinja)
		$(python_abi_depend -e "*-pypy-*" ">=dev-python/pyzmq-2.1.11")
		$(python_abi_depend -e "*-pypy-*" ">=www-servers/tornado-2.1")
	)
...
src_prepare() {
	...
	epatch "${FILESDIR}/${PN}-1.0.0-python3-scripts_versioning.patch"
...
src_install() {
	...
	if use notebook; then
		# Handling of MathJax library based on IPython.html.notebookapp.NotebookApp._mathjax_url_default().
		create_mathjax_symlink() {
			dosym "${EPREFIX}/usr/share/mathjax" "$(python_get_sitedir)/IPython/html/static/mathjax"
		}
